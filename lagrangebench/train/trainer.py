...
def _mse(
    ...
    acc = pred["acc"]
    vel = features["vel_hist"][:,-2:]

    total_momentum = jnp.sum(acc,axis=0)
    momentum_loss= jnp.sum(total_momentum**2)

    energy_derivative = jnp.sum(vel*acc, axis=-1)
    energy_loss = jnp.mean(energy_derivative**2)

    w_momentum = getattr(loss_weight,"momentum",0.0)
    w_energy = getattr(loss_weight, "energy", 0.0)

    losses.append(jnp.ones_like(acc[:,0]) * w_momentum * momentum_loss)
    losses.append(jnp.ones_like(acc[:, 0]) * w_energy * energy_loss)
